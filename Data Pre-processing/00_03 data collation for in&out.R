...
climate<- climate[,c('city_code', 'DATE', 'TEMP','Tmax','Tmin','anomaly.x','anomalys.x','vari.x','high.x','highd','highl','highld','PREC','high.y','high99','high999','anomaly.y','anomalys.y','vari.y')]
colnames(climate) <- c('city_code', 'DATE', 'TEMP','Tmax','Tmin','TEMPa','TEMPas',"tempv",'ipcctemph','ipcctemphd','ipcctempl','ipcctempld','PREC','ipccprec','ipccprec99','ipccprec999','PRECa','PRECas',"precv")
head(climate)
str(climate)
...
## create new variable for dates

# DATE <- unique(climate$DATE[climate$DATE >= 20191230])
# DATE <- as.data.frame(DATE)
# DATE$dates <- ymd(DATE$DATE)
# DATE$year <- year(DATE$dates)
# DATE$month <- month(DATE$dates)
# DATE$ISOweek <- isoweek(DATE$dates)
# DATE$weekday <- wday(DATE$dates, week_start = 1)
# fwrite(DATE, 'data/test data/dates.csv',row.names = F)

# read updated dates
DATE <- read.csv('data/test data/dates_update_14day_lag_20212.csv')
...
# #shenzhen
# data <- data[data$city_code != 440300,]
# #changchun
# data <- data[data$city_code != 220100,]
# #jilin
# data <- data[data$city_code != 220200,]
#shanghai
# data <- data[data$city_code != 310000,]
# #sanya
# data <- data[data$city_code != 460200,]
# #xi'an
# data <- data[data$city_code != 610100,]
# #harbin
# data <- data[data$city_code != 230100,]
# #huhehaote
# data <- data[data$city_code != 150100,]
# #baotou
# data <- data[data$city_code != 150200,]
# #bijie
# data <- data[data$city_code != 520500,]
# #chengdu 
# data <- data[data$city_code != 510100,]
# # xizang
# data <- data[data$prov_code != 54,]
# #xinjiang
# data <- data[data$prov_code != 65,]
...
fwrite(data, 'data/test data/data_all_in&out 21[m].csv', row.names = F)

# 
# data<- read.csv("data/test data/data_all_in&out hk14(2).csv", header = T)
# 
# 
# library(Hmisc)
...
#   geom_vline(aes(xintercept=as.Date('2021-08-01')), colour="#990000", size=0.4)+
#   geom_vline(aes(xintercept=as.Date('2021-08-01')), colour="#990000", size=0.4)+
...
#   # geom_vline(aes(xintercept=as.Date('2021-01-01')), colour="grey40", linetype="dashed", size=0.3)+
...
#   labs(x="date",title='intercity mobility recovery') +
