...
# load best fitting model with incidence dlnm and psi dlnm
...
for (j in 1:length(mod.name))
{
  ...
  ###set fixed incidence
  # srb<-srb[20,1:10]
  # srb.uci<-srb.uci[20,1:10]
  # srb.lci<-srb.lci[20,1:10]
...
write.csv(table1, file = "fig21/ table_high incid inflow.csv", quote = FALSE, row.names = FALSE)
...
mod.name <- c("model1.0", "model1.1", "model1.2", "model1.3")
...
# save relative risk results
write.csv(table1, file = "fig21/table_high incid inflow temp+gdp sr.csv", quote = FALSE, row.names = FALSE)
