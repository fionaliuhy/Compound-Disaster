...
for (i in 1:313){
  ...
  MAE$base[i] <- hydroGOF::mae(data$base.fit[data$city_index == i], 
                               data$intraflow[data$city_index == i], # or data$in_outflow, if test in_outflow
                               na.rm = TRUE)
...
mn <-min(MAE$diff)
mx <-max(MAE$diff)
write.csv(MAE,file = "fig21/mae incidence_temp_density.csv",row.names = F)
...
ggsave(value_map, filename = "fig21/mae incidence_temp_density.pdf")
...
formula <- Y ~ 1 + f(T1, replicate = S1, model = "rw1", cyclic = TRUE,constr = TRUE,
                     scale.model = TRUE,  hyper = precision.prior2) +
  f(S1, model = "bym2", replicate = T2, graph = "output/map.graph",
    scale.model = TRUE, hyper = precision.prior2)+Vh+Vg+basis_cases2 +temp_high_cases_dens4+Vtmax+Vd
...
save(model1, file = "pred/model1_config.RData")
...
if (FALSE) {
    ...
    load(file = "pred/model1_config.RData")
...
for (i in 1:52){
  ...
  # replace mobility data in testing period with NA for out of sample prediction
    casestopred <- data$intrarecovery# response variable
    idx.pred <- which(data$week_index == i)
    casestopred[idx.pred] <- NA # replace mobility in week and day of interest to NA
  ...
  # final model
    ...
    save(preds, file = paste0("pred/preds_",i,"week",".RData"))
}}
...
for (i in 1:52){ # no of weeks # days in a week
    load(paste0("pred/preds_",i,"week",".RData"))
    # idx.pred <- which(data$week_index == i )
...
recovery_ts_prov <- 
  data %>% 
  group_by(week, prov_code) %>%
  # calculate provincial level mobility
  summarise(recovery = mean(intrarecovery),
            fit = mean(pred.mean), 
  ...
  ggplot() + 
  geom_ribbon(aes(x = week, ymin = re.fit.lci, ymax = re.fit.uci), 
              fill = "tomato", alpha = 0.5) + 
  geom_line(aes(x = week, y = re.fit, col = "tomato")) +
  geom_line(aes(x = week, y = re, col = "#499894")) +
  xlab("Week") +
  ylab("Intracity Mobility Intensity Recovery Degree (*100%)") +
  scale_colour_identity(name = "",
                        breaks = c("#499894", "tomato"),
                        labels = c("Observed", "Posterior predicted"),
                        guide = "legend") +
  scale_x_continuous(breaks = c(1,20,40,52), labels = c("1","20","40","52")) +
  scale_y_continuous(breaks = c(0.5, 1.0,2.0), labels = c("0.5", "1.0", "2.0")) + 
  theme_bw() + 
...
ggsave("fig/fig_S05_pred_intra.pdf", height = 30, width = 25, units = "cm")
